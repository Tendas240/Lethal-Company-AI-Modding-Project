#!/usr/bin/env python3
"""Canonicalize the completed C3F18 BMDSFIX1 PR/main integration without arming runtime."""
...
INDEX_PATH = ROOT / "ProfileSources/S1.42AK-BMDSFIX1/FILE_INDEX.json"
PUBLICATION_PATH = ROOT / "BuildSpecs/S1.42AK-BMDSFIX1_BUILD_EVIDENCE/PUBLICATION_VERIFICATION.md"
INTEGRATION_MD = ROOT / "BuildSpecs/S1.42AK-BMDSFIX1_BUILD_EVIDENCE/MAIN_INTEGRATION_CHECKPOINT.md"
INTEGRATION_JSON = ROOT / "BuildSpecs/S1.42AK-BMDSFIX1_BUILD_EVIDENCE/MAIN_INTEGRATION_CHECKPOINT.json"
...
MAIN_MERGE = "21138185f1ab377660b61120f1d07733d4180672"
PR_KA_RUN = 36023454162
SOURCE_RUN = 36023454260
ARCHIVE_RUN = 36023454215
MAIN_KA_RUN = 36024694126
PUBLICATION_RUN = 36017880276
REVIEW_RUN = 36014932493
REVIEW_ARTIFACT = 10813908176
OLD_STATUS = "PHASE_C3F18_BMGHDIAG2_RUNTIME_INCONCLUSIVE_BMDSFIX1_EXACT_PUBLICATION_PASS_INTEGRATION_NEXT"
NEW_STATUS = "PHASE_C3F18_BMGHDIAG2_RUNTIME_INCONCLUSIVE_BMDSFIX1_MAIN_INTEGRATION_PASS_RUNTIME_ACTIVATION_NEXT"
...
require(
    subprocess.run(["git", "merge-base", "--is-ancestor", MAIN_MERGE, "HEAD"], cwd=ROOT).returncode == 0,
    "Expected PR #150 main merge commit is not an ancestor of HEAD",
)
state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
current = json.loads(CURRENT_SPEC_PATH.read_text(encoding="utf-8"))
require(
    state["accepted_baseline"]["build_id"] == "S1.42AK"
    and state["accepted_baseline"]["sha256"] == BASE_SHA,
    "Accepted baseline drift",
)
require(state["latest_built_artifact"]["build_id"] == "S1.42AK", "Latest normal artifact drift")
require(
    state["active_candidate"] is None and state["runtime_test_outstanding"] is False,
    "Runtime lifecycle unexpectedly armed",
)
require(
    current["enabled"] is False
    and current["build_id"] == "IDLE_UNIVERSAL_INTERIOR_VIABILITY_ANALYSIS",
...
review = selected["bmdsfix1_review"]
require(
    review["build_id"] == BUILD_ID
    and review["published"] is True
    and review["main_integrated"] is False
    and review["runtime_armed"] is False,
    "Unexpected pre-integration BMDSFIX1 record",
)
...
dll_rows = [
    row
    for row in index
    if row.get("path") == "BepInEx/plugins/S142AKBMDSFix1/S142AKBMDSFix1.dll"
]
require(
    len(dll_rows) == 1 and dll_rows[0].get("sha256") == DLL_SHA,
    "BMDSFIX1 DLL index identity drift",
)

state["updated"] = "2026-09-24"
selected["status"] = NEW_STATUS
selected["finding"] = (
    f"S1.42AK-BMDSFIX1 exact reviewed bytes are now integrated into main by PR #{PR}, exact head {PR_HEAD}, "
    f"merge commit {MAIN_MERGE}. Exact-head CI passed Knowledge Architecture run {PR_KA_RUN}, "
    f"source/pure-static run {SOURCE_RUN}, and publication-aware archive run {ARCHIVE_RUN}; the archive gate "
    f"used published-validation mode and skipped both rebuild and artifact upload. The resulting main merge "
    f"passed permanent Knowledge Architecture push run {MAIN_KA_RUN}. The main-integrated profile remains "
    f"SHA-256 {PROFILE_SHA}, BMDSFIX1 DLL SHA-256 {DLL_SHA}, with the same 337-member archive/index contract, "
    f"zero package/config drift, LLL {LLL_SHA}, and accepted normalizer {NORMALIZER_SHA}. All live controllers "
    "remain inactive: BuildSpecs/current.json is disabled, RuntimeInbox/ACTIVE_BUILD.txt remains S1.42AK, "
    "active_candidate is null and runtime_test_outstanding is false. The fix remains scoped only to Black Mesa "
    "x DeepSewersFlow. Black Mesa x Greenhouse remains NOT_YET_PROVEN under its separate successor-diagnostic task."
)
selected["analysis_contract"] = (
    "Treat S1.42AK-BMDSFIX1 as exact-byte published and integrated into main, but not runtime-armed and not accepted. "
    f"Any runtime activation must target only profile SHA-256 {PROFILE_SHA} / BMDSFIX1 DLL SHA-256 {DLL_SHA} directly "
    "over accepted S1.42AK and must be a separate atomic lifecycle/controller transition. Until that later step, keep "
    "BuildSpecs/current.json disabled, RuntimeInbox/ACTIVE_BUILD.txt at S1.42AK, active_candidate null and "
    "runtime_test_outstanding=false. Preserve S1.42AB InteriorWeightNormalization, Dawn/native Black Mesa ownership, "
    "Greenhouse availability, the B3 matrix, Shatteredrooms exclusions, Black-Mesa/Pikmin routing and the separate "
    "Black Mesa x Greenhouse diagnostic scope."
)
next_action = (
    f"Prepare a separate atomic runtime-activation checkpoint for the exact main-integrated S1.42AK-BMDSFIX1 profile "
    f"SHA-256 {PROFILE_SHA} / BMDSFIX1 DLL SHA-256 {DLL_SHA}. That later activation may point "
    "RuntimeInbox/ACTIVE_BUILD.txt at S1.42AK-BMDSFIX1 and set runtime_test_outstanding=true only while preserving "
    "accepted/latest normal baseline S1.42AK and keeping BuildSpecs/current.json disabled. Do not change gameplay, "
    "config or package bytes. The activation response must provide the repository-derived Gale replacement/import "
    "command and exact build-specific one-line PowerShell runtime-log uploader. Black Mesa x Greenhouse successor-"
    "diagnostic repair remains separate."
)
...
review.update(
    {
        ...
        "integration_knowledge_architecture_run": PR_KA_RUN,
        "integration_source_static_run": SOURCE_RUN,
        "integration_archive_gate_run": ARCHIVE_RUN,
        "main_knowledge_architecture_run": MAIN_KA_RUN,
        "integration_checkpoint": "BuildSpecs/S1.42AK-BMDSFIX1_BUILD_EVIDENCE/MAIN_INTEGRATION_CHECKPOINT.md",
...
checkpoint = {
    ...
    "pr_knowledge_architecture_run": PR_KA_RUN,
...
INTEGRATION_MD.write_text(
    f"""# S1.42AK-BMDSFIX1 main-integration checkpoint
...
- PR Knowledge Architecture: run `{PR_KA_RUN}` — PASS;
- BMDSFIX1 source/pure-static gate: run `{SOURCE_RUN}` — PASS;
- BMDSFIX1 publication-aware archive gate: run `{ARCHIVE_RUN}` — PASS;
- permanent main Knowledge Architecture push gate: run `{MAIN_KA_RUN}` — PASS.
...
- Knowledge Architecture run `{PR_KA_RUN}`;
- BMDSFIX1 source/pure-static run `{SOURCE_RUN}`;
- publication-aware archive-delta run `{ARCHIVE_RUN}`.

The resulting merge then passed permanent `main` Knowledge Architecture push run `{MAIN_KA_RUN}`.
...
BMDSFIX1 is now repository-published on `main`, but is still not Gale-imported, not runtime-armed and not accepted. Runtime activation is the next separately bounded atomic checkpoint.
...
lifecycle = replace_section(
    lifecycle,
    "## BMDSFIX1 exact-byte publication — pass / integration next",
    "## Live execution state",
    f"""## BMDSFIX1 exact-byte publication — main integration pass

The pair-scoped Deep Sewers mitigation `S1.42AK-BMDSFIX1` has passed source/static review, the separate inactive review build, exact-byte publication, and main integration. PR #{PR} exact head `{PR_HEAD}` merged at `{MAIN_MERGE}` after Knowledge Architecture run `{PR_KA_RUN}`, source/pure-static run `{SOURCE_RUN}`, and publication-aware archive run `{ARCHIVE_RUN}` all passed. The resulting main merge passed permanent Knowledge Architecture push run `{MAIN_KA_RUN}`.

The integration archive gate selected published-validation mode: no profile rebuild and no Actions artifact upload occurred. The exact main-integrated profile SHA-256 remains `{PROFILE_SHA}` and BMDSFIX1 DLL SHA-256 remains `{DLL_SHA}`. The 337-member archive/index contract, zero package/config drift, LLL `{LLL_SHA}`, accepted normalizer `{NORMALIZER_SHA}`, and reconstructed readable ProfileSources snapshot remain unchanged. Integration evidence: `BuildSpecs/S1.42AK-BMDSFIX1_BUILD_EVIDENCE/MAIN_INTEGRATION_CHECKPOINT.md`.
