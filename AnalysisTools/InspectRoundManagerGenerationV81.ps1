...
$EvidenceRoot = 'SourceEvidence/VanillaV81/RoundManagerGeneration'
$ReportName = 'ROUNDMANAGER_GENERATION_FOCUSED_DECOMPILE.txt'
$Utf8 = New-Object System.Text.UTF8Encoding($false)
$RequiredMethods = @(
    'GenerateNewLevelClientRpc',
    'GenerateNewFloor'
)
function Write-Step { param([string]$Message) Write-Host ('[RoundManagerGenerationV81] ' + $Message) -ForegroundColor Cyan }
...
function New-EvidenceTreeEntries {
    ...
    if ($Directory -notmatch '^SourceEvidence/VanillaV81/RoundManagerGeneration/[0-9TZ]+-[a-f0-9]+$') { throw 'Unexpected publication directory.' }
    return @(
...
'@
    ...
    $entries = @(New-EvidenceTreeEntries -Directory 'SourceEvidence/VanillaV81/RoundManagerGeneration/20260920T000000Z-abcdef12' -Report 'report' -Manifest '{}')
...
$script:CaptureTempRoot = Join-Path ([IO.Path]::GetTempPath()) ('lc-roundmanager-generation-v81-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    [void]$builder.AppendLine('# Installed Lethal Company V81 RoundManager generation-gate evidence')
    ...
    [void]$builder.AppendLine('Scope: GenerateNewLevelClientRpc, GenerateNewFloor and one-hop direct callers within RoundManager.')
    ...
    $branch = 'source-evidence/roundmanager-generation-v81-' + $stamp.ToLowerInvariant() + '-' + $suffix
    $metadata = [ordered]@{
        schema_version = 1
        purpose = 'Native V81 RoundManager generation-gate evidence for Universal Interior Viability Phase C3E3F'
    ...
    $commitResult = Invoke-RepoApi -Endpoint 'git/commits' -Method 'POST' -Body @{
        message = ('Capture exact V81 RoundManager generation-gate evidence ' + $stamp)
        tree = $treeResult.sha
