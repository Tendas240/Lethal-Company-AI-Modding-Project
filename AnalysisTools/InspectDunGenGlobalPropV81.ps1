...
$EvidenceRoot = 'SourceEvidence/VanillaV81/DunGenGlobalProp'
$ReportName = 'DUNGEN_GLOBALPROP_FOCUSED_DECOMPILE.txt'
...
$GlobalPropSignalPattern = '\bGlobalProps?\b|\bPropGroupID\b|\bGlobalPropID\b'
$CountSignalPattern = '\bCount\b'
$Utf8 = New-Object System.Text.UTF8Encoding($false)

function Write-Step {
    param([string]$Message)
    Write-Host ('[DunGenGlobalPropV81] ' + $Message) -ForegroundColor Cyan
}
...
function Get-SignalMethods {
    param(
        [Parameter(Mandatory = $true)][string]$Source,
        [Parameter(Mandatory = $true)][string]$ClassName,
        [Parameter(Mandatory = $true)][string]$SignalPattern,
        [Parameter(Mandatory = $true)][string]$RequiredSecondaryPattern,
        [int]$MaxLines = 3000
    )
    ...
    if ($methods.Count -eq 0) { throw 'No top-level methods were parsed.' }
    $primary = @($methods | Where-Object { $_.Body -match $SignalPattern })
    if ($primary.Count -eq 0) { throw 'No DungeonGenerator method references the GlobalProp signal surface.' }

    $primaryNames = @($primary | ForEach-Object { $_.Name } | Sort-Object -Unique)
    $selected = @($primary)
    foreach ($method in $methods) {
        if ($primaryNames -contains $method.Name) { continue }

        $isCaller = $false
        foreach ($name in $primaryNames) {
            if ($method.Body -match ('\b' + [regex]::Escape($name) + '\s*\(')) { $isCaller = $true; break }
        }

        $isCallee = $false
        foreach ($p in $primary) {
            if ($p.Body -match ('\b' + [regex]::Escape($method.Name) + '\s*\(')) { $isCallee = $true; break }
        }
    ...
    if (@($selected | Where-Object { $_.Body -match $RequiredSecondaryPattern }).Count -eq 0) {
        throw 'GlobalProp-focused method neighborhood does not expose Count.'
    }

    $lineCount = 0
    foreach ($method in $selected) {
        if ($method.Body -match '^\s*throw\s+null\s*;\s*$') { throw ('Reference-only throw-null stub rejected: ' + $method.Name) }
        $lineCount += @($method.Text -split '\r?\n').Count
    }
    if ($lineCount -gt $MaxLines) { throw "Focused extraction is $lineCount lines; limit $MaxLines." }
    return $selected
}

function New-EvidenceTreeEntries {
    param([string]$Directory, [string]$Report, [string]$Manifest)
    if ($Directory -notmatch '^SourceEvidence/VanillaV81/DunGenGlobalProp/[0-9TZ]+-[a-f0-9]+$') {
...
function Invoke-ProvenanceSelfTest {
    ...
    $bad = $valid.Clone()
    $bad.ManifestSha = 'unreviewed'
    $failed = $false
    try { Assert-InstalledGameProvenance @bad } catch { $failed = $true }
    if (-not $failed) { throw 'Unknown manifest was not rejected.' }
}
...
public class DungeonGenerator {
    ...
    {
        foreach (var globalProp in DungeonFlow.GlobalProps)
        {
            int count = globalProp.Count.GetRandom(randomStream);
            PlaceGlobalProp(globalProp, count);
        }
    }
    private void PlaceGlobalProp(object prop, int count)
    {
        if (count > 0) { count--; }
    }
    ...
    {
        var text = "GlobalProps Count";
    }
}
'@
    $result = @(Get-SignalMethods -Source $fixture -ClassName 'DungeonGenerator' -SignalPattern $GlobalPropSignalPattern -RequiredSecondaryPattern $CountSignalPattern)
    $names = @($result | ForEach-Object { $_.Name })
    if ($names -notcontains 'ProcessGlobalProps' -or $names -notcontains 'PlaceGlobalProp' -or $names -notcontains 'Generate' -or $names -contains 'Unrelated') {
        throw 'Signal/caller/callee selection self-test failed.'
    }

    $failed = $false
    try {
        Get-SignalMethods -Source $fixture -ClassName 'DungeonGenerator' -SignalPattern '\bDefinitelyAbsent\b' -RequiredSecondaryPattern $CountSignalPattern | Out-Null
    }
    catch { $failed = $true }
    if (-not $failed) { throw 'Missing GlobalProp signal was not rejected.' }

    $failed = $false
    try {
        Get-SignalMethods -Source $fixture -ClassName 'DungeonGenerator' -SignalPattern $GlobalPropSignalPattern -RequiredSecondaryPattern '\bDefinitelyAbsent\b' | Out-Null
    }
    catch { $failed = $true }
    if (-not $failed) { throw 'Missing Count signal was not rejected.' }

    $entries = @(New-EvidenceTreeEntries -Directory 'SourceEvidence/VanillaV81/DunGenGlobalProp/20260921T000000Z-abcdef12' -Report 'report' -Manifest '{}')
...
function Invoke-BootstrapSelfTest {
    $temp = Join-Path ([IO.Path]::GetTempPath()) ('lc-dungen-bootstrap-' + [guid]::NewGuid().ToString('N'))
...
if ($SelfTest) {
    ...
    Write-Host 'PASS: provenance and focused extractor self-tests.'
    return
}
...
$script:CaptureTempRoot = Join-Path ([IO.Path]::GetTempPath()) ('lc-dungen-globalprop-v81-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $script:CaptureTempRoot -Force | Out-Null

try {
    Write-Step 'Locating and verifying installed V81.'
    ...
    if ($pullRequest.state -cne 'open' -or $pullRequest.merged -eq $true -or $pullRequest.head.ref -cne $PullRequestBranch -or $pullRequest.head.repo.full_name -cne $RepositoryName) {
        throw 'PR #138 is not the expected open in-repository evidence PR.'
    }
    ...
    if ($prior.source_assembly.sha256 -ne $ExpectedAssemblySha256 -or
        ...
        $prior.steam.appmanifest_sha256 -ne $ExpectedAppManifestSha256) {
        throw 'Current main prior evidence disagrees with the pinned installed-game contract.'
    }
    ...
    $source = Invoke-CheckedNativeProcess -FilePath $ilspy.DotNet -Arguments @(
        $ilspy.IlSpyDll, '-t', 'DunGen.DungeonGenerator', '-r', $managedDir, $dunGenPath
    ) -Label 'DunGen.DungeonGenerator decompile'
    if ([string]::IsNullOrWhiteSpace($source)) { throw 'Decompiler returned empty source.' }

    $methods = @(Get-SignalMethods -Source $source -ClassName 'DungeonGenerator' -SignalPattern $GlobalPropSignalPattern -RequiredSecondaryPattern $CountSignalPattern)

    $builder = New-Object Text.StringBuilder
    [void]$builder.AppendLine('# Installed Lethal Company V81 DunGen GlobalProp generation evidence')
    ...
    [void]$builder.AppendLine('Scope: DunGen.DungeonGenerator methods referencing GlobalProp/GlobalProps/PropGroupID plus one-hop same-type callers and callees. At least one selected method must expose Count.')
    [void]$builder.AppendLine('This is static installed-binary evidence, not gameplay acceptance. DunGen.dll, full type source, unrelated methods and absolute local paths are excluded.')
    foreach ($method in $methods) {
        [void]$builder.AppendLine('')
    ...
    $metadata = [ordered]@{
        schema_version = 1
        purpose = 'Exact installed-V81 DunGen GlobalProp count/selection generation semantics for Universal Interior Viability C3 count-three gate'
        ...
        dungen_assembly = @{
            ...
            sha256 = $dunGenSha
            provenance_note = 'Exact installed file colocated with the SHA-bound V81 Assembly-CSharp.dll; this capture establishes its repository-visible byte binding.'
        }
        ...
        decompiler = @{
            ...
            full_local_type_source_sha256 = (Get-TextSha256 $source)
        }
        selection = @{
            primary_signal_pattern = $GlobalPropSignalPattern
            required_secondary_pattern = $CountSignalPattern
            selected_signatures = @($methods | ForEach-Object { $_.Signature })
            same_type_neighbor_depth = 1
            neighbor_direction = @('callers', 'callees')
            max_source_lines = 3000
        }
        report = @{
            file = $ReportName
            sha256 = (Get-TextSha256 $report)
            excludes = @('game binaries', 'DunGen.dll', 'full type decompile', 'unrelated methods', 'absolute local paths', 'user names')
        }
    }
    ...
    Write-Step ('Publishing ' + $methods.Count + ' focused method blocks to PR #138.')
    ...
    if ($currentPullRequest.state -cne 'open' -or
        ...
        $currentPullRequest.head.sha -cne $repositoryPrHead) {
    ...
    $commitResult = Invoke-RepoApi -Endpoint 'git/commits' -Method 'POST' -Body @{
        message = ('Capture exact V81 DunGen GlobalProp evidence ' + $stamp)
        tree = $treeResult.sha
