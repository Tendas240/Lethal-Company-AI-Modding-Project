...
require(output.is_file(), "Builder did not produce diagnostic output profile")
...
index_rows = [row for row in index if row.get("path") == DIAGNOSTIC_DLL]
require(len(index_rows) == 1, "Diagnostic DLL missing or duplicated in FILE_INDEX")
require(index_rows[0]["sha256"] == diagnostic_hash, "FILE_INDEX diagnostic DLL hash mismatch")
...
dest = ROOT / "BuildSpecs/S1.42AK-BMGHDIAG1_BUILD_EVIDENCE"
