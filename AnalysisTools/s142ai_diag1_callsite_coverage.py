...
REQUIRED_SOURCE_RUNS = {
    "34529045495", "34614454120", "34616289395", "34705834727",
    "34708556035", "34709775849", "34710724733", "34711742173",
    "34713018420", "34714066827", "34714900794", "34716488701",
}
...
# These are evidence-backed runtime/config gates from the exact package reviews.
RUNTIME_GATED_RULES = [
...
# Exact Shy Guy owners are allowed by identity, not by location. Exterior Shy Guy must remain observable.
ALLOW_SHYGUY_RULES = [
    ("Scopophobia", "ShyGuyPaintingProp", "SpawnEnemyOnServer"),
    ("BCMER", "ShyGuy", "Execute"),
    ("BrutalCompany", "ShyGuy", "Execute"),
]
...
def class_context(lines: list[str], idx: int) -> tuple[str, bool]:
    # Nearest preceding class/struct declaration is sufficient for generated exact-source inventory.
...
def method_name_from_header(header: str) -> str | None:
    ...
    if lower.startswith(CONTROL_PREFIXES):
        return None
    # Drop attributes and generic constraints noise; choose token immediately before '('.
    pre = h.split("(", 1)[0].strip()
    token = re.split(r"\s+", pre)[-1]
    token = token.split(".")[-1]
    token = token.split("<", 1)[0]
    token = token.strip()
    if not re.match(r"^[A-Za-z_][\w`]*$", token):
        return None
    if token in {"new", "return", "typeof", "nameof", "default"}:
        return None
    return token
...
def extract_cs_methods(text: str) -> list[dict]:
    ...
    i = 0
    while i < len(lines):
        # Accumulate short multi-line headers ending at an opening brace.
        if "(" not in lines[i]:
            i += 1
            continue
        start = i
        ...
        for j in range(brace_pos_line, len(lines)):
            line = lines[j]
            # Good enough for decompiler output: source artifacts are generated C#, and method extraction is
            # corroborated by IL artifacts. Braces in strings are rare and only widen a block, never silently
            # remove a primitive hit; body hashes make the result auditable.
            opens = line.count("{")
            closes = line.count("}")
            if opens:
                seen = True
        ...
        out.append({
            "method": name,
            "class": cls,
            "class_enemy_ai_like": enemy_ai,
            "signature": " ".join(header.split())[:600],
            "start_line": start + 1,
            "end_line": end + 1,
            "body": body,
        })
...
def extract_il_methods(text: str) -> list[dict]:
    ...
    while i < len(lines):
        ...
        out.append({
            "method": name,
            "class": "<il>",
            "class_enemy_ai_like": False,
            "signature": header[:600],
            "start_line": start + 1,
            "end_line": end + 1,
            "body": body,
        })
...
def is_relevant(flags: list[str], body: str) -> bool:
    f = set(flags)
    if f & {
        "roundmanager_spawn_enemy_game_object", "roundmanager_spawn_enemy_on_server",
    ...
    if "enemy_prefab_instantiate" in f and ("network_spawn" in f or "enemytype_reference" in f):
        return True
    # Vent assignment is relevant only when it actually handles EnemyType identity/state.
    if "vent_enemy_assignment" in f and "enemytype_reference" in f:
        return True
    return False
...
def source_tag(path: Path) -> tuple[str, str]:
    parts = path.parts
    run_id = next((p for p in parts if p in RUN_LABELS), "repo")
    label = RUN_LABELS.get(run_id, "repository_source")
    return run_id, label
...
def classify(row: dict) -> tuple[str, str]:
    ...
    method = row["method"]
    sig = row["signature"]
    joined = f"{file_name} {cls} {method} {sig}"

    # Exact Shy Guy creation is intentionally allowed; location must not affect this decision.
    for src, c, m in ALLOW_SHYGUY_RULES:
        if src.lower() in joined.lower() and c.lower() in joined.lower() and m.lower() in method.lower():
    ...
    for src, c, m, why in RUNTIME_GATED_RULES:
        if src.lower() in joined.lower() and c.lower() in joined.lower() and (not m or m.lower() in method.lower()):
            return "RUNTIME_GATED_OR_DORMANT", why

    if row.get("class_enemy_ai_like") and "enemy_prefab_instantiate" in row["primitive_flags"]:
        return "PARENT_PREVENTION_CANDIDATE", "direct child spawn owned by an EnemyAI-like parent; prove parent cannot exist under ShyGuy-only guard before omitting a child guard"

    # BCMER direct monster-spawn helpers are constrained by the already-closed ShyGuy-only event config/execution gate.
...
def iter_source_files(artifacts_root: Path, repo_root: Path) -> Iterable[tuple[Path, str]]:
    for p in sorted(artifacts_root.rglob("*")):
        if p.is_file() and p.suffix.lower() in {".cs", ".il"}:
            yield p, "artifact"
    native_files = [
...
def main() -> int:
    ...
    for path, kind in iter_source_files(artifacts_root, repo_root):
        ...
        source_inventory.append({
            "path": str(path.relative_to(repo_root) if kind == "native_v81" else path.relative_to(artifacts_root)),
            "kind": kind,
            "run_id": run_id,
            "run_label": run_label,
            "sha256": digest,
        ...
        # Exact artifacts can repeat a source file across later review runs; analyze identical bytes once.
        ...
        # Native RoundManager capture is a text concatenation of C# method blocks.
        if kind == "native_v81" and path.suffix.lower() == ".txt":
            methods = extract_cs_methods(text)
        for m in methods:
            flags = primitive_flags(m["body"])
            if not is_relevant(flags, m["body"]):
                continue
            row = {
                "source_file": path.name,
                "source_path": source_inventory[-1]["path"],
                "source_sha256": digest,
                "source_kind": kind,
                "run_id": run_id,
                "run_label": run_label,
                "class": m["class"],
                "class_enemy_ai_like": m["class_enemy_ai_like"],
                "method": m["method"],
            ...
            row["coverage_rationale"] = rationale
            rows.append(row)

    # Stable de-duplication by exact source+method body; retains separate package versions/files where bytes differ.
    dedup = {}
    for row in rows:
    ...
    matrix = {
        "schema_version": 1,
        "scope": "S1.42AI-DIAG1 exact enemy-spawn callsite coverage",
        "analysis_only": True,
        "base_contract": "BuildSpecs/S1.42AI_PLAN.md",
        "source_runs": RUN_LABELS,
        "source_file_count": len(source_inventory),
        "unique_relevant_callsite_count": len(rows),
    ...
    md = [
        "# S1.42AI-DIAG1 Exact Spawn Callsite Coverage Matrix",
        "",
        "**Status:** ANALYSIS-ONLY / NO IMPLEMENTATION / NO BUILD AUTHORIZATION",
        "",
        f"Unique relevant callsites: **{len(rows)}**",
        f"Source files inventoried: **{len(source_inventory)}**",
    ...
    for r in rows:
        flags = ", ".join(r["primitive_flags"])
        md.append(f"| `{r['coverage_category']}` | `{r['source_file']}` | `{r['class']}` | `{r['method']}` | {flags} |")
    md += [
        "",
