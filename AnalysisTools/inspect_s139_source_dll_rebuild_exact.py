...
def git_show(commit: str, path: str) -> bytes:
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"git show failed for {commit}:{path} with exit code {proc.returncode}\n"
...
def extract_member(profile_path: Path, expected_profile_sha: str, destination: Path) -> dict:
    ...
    with zipfile.ZipFile(profile_path, "r") as archive:
        matches = [info for info in archive.infolist() if info.filename == DLL_MEMBER]
        if len(matches) != 1:
...
def write_summary(output_dir: Path, verification: dict) -> None:
    ...
    lines = [
        ...
        f"- Rebuilt DLL SHA-256: `{comparison.get('rebuilt_sha256', '<unavailable>')}`",
        f"- Byte-identical: `{comparison.get('byte_identical', False)}`",
        f"- Full C# decompile identical: `{comparison.get('csharp_identical', False)}`",
        f"- Full IL decompile identical: `{comparison.get('il_identical', False)}`",
        "",
...
def main() -> int:
    ...
    bin_dir = ROOT / "Patches/S139CompatibilityFixes/bin"
    obj_dir = ROOT / "Patches/S139CompatibilityFixes/obj"
    ...
    try:
        ...
        historical_helper = git_show(PREBUILD_COMMIT, HISTORICAL_HELPER_REL).decode("utf-8")
        historical_builder = git_show(PREBUILD_COMMIT, PROFILE_BUILDER_REL).decode("utf-8")
        historical_workflow = git_show(PREBUILD_COMMIT, HISTORICAL_WORKFLOW_REL).decode("utf-8")
        ...
        missing_helper = [m for m in required_helper_markers if m not in historical_helper]
        if missing_helper:
            raise RuntimeError(f"Historical S1.42AH helper contract drift/missing markers: {missing_helper}")
        exact_builder_marker = 'subprocess.run(["dotnet", "build", str(project), "-c", config], check=True)'
        if exact_builder_marker not in historical_builder:
            raise RuntimeError("Historical profile builder no longer proves exact dotnet build <project> -c <config> path")
        for marker in ['runs-on: ubuntu-latest', 'dotnet-version: "8.0.x"', 'python-version: "3.12"']:
            if marker not in historical_workflow:
                raise RuntimeError(f"Historical successful workflow toolchain marker missing: {marker}")
        ...
        verification["source_lineage"] = {
            ...
            "historical_helper_sha256": sha_bytes(historical_helper.encode("utf-8")),
            "historical_profile_builder_sha256": sha_bytes(historical_builder.encode("utf-8")),
            "historical_workflow_sha256": sha_bytes(historical_workflow.encode("utf-8")),
        }
        ...
        dotnet_info = require_success(run(["dotnet", "--info"]), "dotnet --info")
        (output_dir / "DOTNET_INFO.txt").write_text(dotnet_info, encoding="utf-8")
        ...
        build_proc = run(["dotnet", "build", CSPROJ_REL, "-c", "Release"])
        ...
        require_success(build_proc, "exact S139 Release rebuild")

        built_dll = ROOT / BUILT_DLL_REL
        if not built_dll.exists():
            raise RuntimeError(f"Expected rebuilt DLL missing: {BUILT_DLL_REL}")
        ...
        ilspy_version_proc = run([str(ilspy), "--version"])
        ilspy_version = require_success(ilspy_version_proc, "ilspycmd --version").strip()
        (output_dir / "ILSPY_VERSION.txt").write_text(
            ilspy_version + ("\n" if not ilspy_version.endswith("\n") else ""),
            encoding="utf-8",
        )
        ...
        runtime_for_compare = ai_ref
        runtime_csharp, runtime_csharp_err = decompile(ilspy, runtime_for_compare, il=False)
        rebuilt_csharp, rebuilt_csharp_err = decompile(ilspy, rebuilt_copy, il=False)
        runtime_il, runtime_il_err = decompile(ilspy, runtime_for_compare, il=True)
        ...
        runtime_bytes = runtime_for_compare.read_bytes()
        ...
        verification["comparison"] = {
            ...
            "csharp_identical": csharp_identical,
            "runtime_il_sha256": sha_bytes(runtime_il.encode("utf-8")),
            "rebuilt_il_sha256": sha_bytes(rebuilt_il.encode("utf-8")),
            "il_identical": il_identical,
        }

        if byte_identical:
            verification["status"] = "PASS_BYTE_IDENTICAL"
            verification["source_to_dll_provenance"] = "CLOSED_EXACT_BYTE_REBUILD"
            rc = 0
        elif csharp_identical and il_identical:
            verification["status"] = "PASS_FULL_CSHARP_AND_IL_IDENTICAL_BYTE_NONIDENTICAL"
            verification["source_to_dll_provenance"] = "CLOSED_FULL_DECOMPILE_EQUIVALENCE"
            rc = 0
        else:
            verification["status"] = "FAIL_REBUILD_NOT_EQUIVALENT"
