...
Perform a bounded repository-native installed normal-stack seed-control availability review for the exact S1.42AK-BMDSFIX1 profile. Determine whether any already-installed ordinary non-diagnostic package/config/plugin mechanism can deliberately supply map seed 1061420 through the native StartOfRound/RoundManager path without adding new code, selecting the dungeon directly, mutating the viable pool/weights, or consuming LevelRandom before native selection. The exact Vanilla StartOfRound capture is reconciled and contains no Assembly-CSharp writer for overrideRandomSeed / overrideSeedNumber. If an existing ordinary mechanism is proven, reconcile it before releasing the regular Black Mesa x DeepSewersFlow target gameplay test; if none exists, record the limitation and keep the target gate outstanding without blind rerolls.
