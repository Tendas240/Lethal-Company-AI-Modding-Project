...
Corrected migrated SHA-256: `0245e0c5551d77cab7f90eacf7f1627b0a6df62553260533a4e903d2a6426f27`

Historical note: `ab894ead...` was an earlier pre-correction S1.40A archive variant and is not the canonical migrated S1.40A binary.
...
GitHub Actions hash-verified and indexed the uploaded canonical S1.41 profile.

Runtime acceptance on 2026-09-02:
- exact BCMER 1.71.0 loaded and finished patching;
- normal events were selected/executed (Arachnophobia, ScarceOutsideScrap, with LeaflessTrees as an additional event);
- post-run ownership guard values survived;
- Raining, HeavyRain, AllWeather and Hurricane stayed disabled;
...
A separate non-blocking issue was observed in the Mineshaft: with many Pikmin in the moving elevator, the player clipped through the floor and died from fall damage. The nearby log window contains many NavMesh-agent creation failures. Causality is not proven and BCMER is not implicated.
...
S1.41 is now the frozen accepted BCMER baseline.

Next binding stage:
`S1.42A Interior Config Seed`

Do not pre-tune guessed interior IDs. Add the binding interior packages after a fresh dependency/version audit, generate real runtime config sections/IDs, collect the full config directory plus LogOutput, then tune S1.42.
