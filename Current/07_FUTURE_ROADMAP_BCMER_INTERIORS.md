...
**S1.40A test -> if Currency + Flash Turret pass -> S1.41 BCMER 1.71.0 build -> S1.41 test -> S1.42A Interior Config Seed -> runtime config generation -> collect full config + LogOutput -> analyze/tune -> S1.42 final interior build -> S1.42 test.**
...
First reactivation must use **exact existing 1.71.0**.
...
Do not combine any interior additions with S1.41.
