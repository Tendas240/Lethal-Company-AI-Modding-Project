...
The artifact-integrity registry therefore deliberately retains both unresolved profile artifacts: DIAG1 as the active pending runtime candidate, and S1.42AI under the dedicated deferred-pending role `DEFERRED_FULL_NORMAL_RUNTIME_GATE_NOT_WAIVED`. The byte-integrity validator explicitly recognizes both roles while still requiring exact profile bytes, SHA, readable snapshot and candidate evidence for each.
