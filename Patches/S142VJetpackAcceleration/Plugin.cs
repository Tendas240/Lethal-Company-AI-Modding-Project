...
namespace S142VJetpackAcceleration
{
    ...
    {
        internal const string PluginGuid = "tendas.lethalcompany.s142vjetpackacceleration";
        internal const string PluginName = "S1.42V Jetpack Acceleration";
        ...
        private void Awake()
        {
            ...
            {
                Logger.LogError("S1.42V Jetpack acceleration patch refused to arm because the validated dependency set does not match the frozen S1.42U baseline.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42V Jetpack acceleration patch refused to arm: exact JetpackItem.Update() target was not found with the expected void signature.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42V Jetpack acceleration patch refused to arm: expected ButteRyBalance JetpackItem.Update prefix owner is missing.");
                return;
            }

            if (!existing.Transpilers.Any(p => p.owner == JetpackFixesGuid))
            {
                Logger.LogError("S1.42V Jetpack acceleration patch refused to arm: expected JetpackFixes JetpackItem.Update transpiler owner is missing.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42V Jetpack acceleration patch refused to arm: More Ship Upgrades is loaded but its expected JetpackItem.Update transpiler owner is missing.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42V Jetpack acceleration patch failed post-patch verification; removing project-local patches.");
                harmony.UnpatchSelf();
                return;
            }

            Logger.LogInfo(
                "S1.42V Jetpack acceleration patch armed: JetpackItem.Update() local-player base acceleration 10 -> 12, " +
    ...
    internal static class JetpackAccelerationPatch
    {
        ...
        private const float TargetBaseAcceleration = 12f;
        ...
        private static void Prefix(JetpackItem __instance)
        {
            ...
            // ButteRyBalance v0.7.0 with the frozen S1.42U settings (V49, Warmup=false)
            // writes exactly 10f immediately before the original JetpackItem.Update body.
            // Only replace that proven baseline value. Any other value is left untouched so
            // warmup/dynamic/future owners are not broadly overridden.
