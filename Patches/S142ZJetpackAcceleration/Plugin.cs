...
namespace S142ZJetpackAcceleration
{
    ...
    {
        internal const string PluginGuid = "tendas.lethalcompany.s142zjetpackacceleration";
        internal const string PluginName = "S1.42Z Jetpack Acceleration";
        ...
        private void Awake()
        {
            ...
            {
                Logger.LogError("S1.42Z Jetpack acceleration patch refused to arm because the validated dependency set does not match the frozen full-stack baseline.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42Z Jetpack acceleration patch refused to arm: exact JetpackItem.Update() target was not found with the expected void signature.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42Z Jetpack acceleration patch refused to arm: expected ButteRyBalance JetpackItem.Update prefix owner is missing.");
                return;
            }

            if (!existing.Transpilers.Any(p => p.owner == JetpackFixesGuid))
            {
                Logger.LogError("S1.42Z Jetpack acceleration patch refused to arm: expected JetpackFixes JetpackItem.Update transpiler owner is missing.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42Z Jetpack acceleration patch refused to arm: More Ship Upgrades is loaded but its expected JetpackItem.Update transpiler owner is missing.");
                return;
            }
            ...
            {
                Logger.LogError("S1.42Z Jetpack acceleration patch failed post-patch verification; removing project-local patches.");
                harmony.UnpatchSelf();
                return;
            }

            Logger.LogInfo(
                "S1.42Z Jetpack acceleration patch armed: JetpackItem.Update() local-player base acceleration 10 -> 18, " +
    ...
    internal static class JetpackAccelerationPatch
    {
        ...
        private const float TargetBaseAcceleration = 18f;
