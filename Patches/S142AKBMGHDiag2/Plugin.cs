...
{
    [BepInPlugin(Guid, "S1.42AK-BMGHDIAG2 Black Mesa Greenhouse Diagnostic", "1.0.0")]
    ...
    {
        internal const string Guid = "tendas.lethalcompany.s142akbmghdiag2";
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                Logger.LogInfo("[BMGHDIAG2] ARMED installed V81 source-hash provenance + loaded TeleportPlayer structural identity passed; exact LLL selection + read-only observer; Black Mesa / Greenhouse only; DIAGNOSTIC ONLY, NEVER ACCEPT.");
                Logger.LogInfo("[BMGHDIAG2] Selection postfix owners: " + string.Join(", ", installedSelection.Postfixes.Select(p => p.owner)));
                Logger.LogInfo("[BMGHDIAG2] TeleportPlayer postfix owners: " + string.Join(", ", installedTeleport.Postfixes.Select(p => p.owner)));
            }
            catch (Exception ex)
            {
                harmony?.UnpatchSelf();
                Logger.LogError("[BMGHDIAG2] REFUSED TO ARM; normal behavior preserved: " + ex);
            }
        }
        ...
        private static void ResolveObservationContract()
        {
            ...
            Require(entranceTeleportType != null, "Exact EntranceTeleport runtime type is missing.");
            Assembly runtimeAssembly = entranceTeleportType.Assembly;
            Require(runtimeAssembly != null && runtimeAssembly.ManifestModule != null,
                "EntranceTeleport runtime assembly/module is missing.");
            ...
            Require(teleportTarget != null && teleportTarget.DeclaringType == entranceTeleportType,
                "Exact declared EntranceTeleport.TeleportPlayer() is missing.");
            RuntimeIdentityPolicy.ValidateTeleportMethodIdentity(
                teleportTarget.IsPublic,
                teleportTarget.IsStatic,
                teleportTarget.GetParameters().Length,
                teleportTarget.ReturnType.FullName,
        ...
        private static Assembly Dependency(string guid, string version, string hash)
        {
            ...
            ValidateDependencyAssemblyHash(assembly, hash, guid);
            return assembly;
        }

        private static void ValidateDependencyAssemblyHash(Assembly assembly, string expected, string label)
        {
            string location = assembly.Location;
            Require(!string.IsNullOrWhiteSpace(location) && File.Exists(location), "Cannot hash loaded dependency assembly: " + label);
            using (SHA256 sha = SHA256.Create())
            using (FileStream stream = File.OpenRead(location))
            {
                string actual = BitConverter.ToString(sha.ComputeHash(stream)).Replace("-", "").ToLowerInvariant();
                Require(actual == expected, "Loaded dependency assembly SHA-256 mismatch: " + label + " got " + actual);
            }
        }
        ...
        private static void SelectionPostfix(object __result, object __0, bool __1)
        {
            ...
            try
            {
                ...
                self.Logger.LogInfo("[BMGHDIAG2] SELECTED Black Mesa Greenhouse / GreenhouseFlow; normalized rarity=100; pool="
                    + count + "->1; same viable wrapper; normalizer -> diagnostic singleton; DIAGNOSTIC ONLY.");
            }
            catch (Exception ex)
            {
                runtimeRefused = true;
                self.Logger.LogError("[BMGHDIAG2] REFUSED selection; diagnostic invalid, returned list left unmodified unless all validation had already completed: " + ex);
            }
        }
        ...
        private static void TeleportPostfix(object __instance)
        {
            ...
            try
            {
                ...
                {
                    self.Logger.LogInfo("[BMGHDIAG2] TRAVERSAL_IGNORED id=" + sourceId + " outside required diagnostic IDs 0..3.");
                    return;
                }
                if (decision == TraversalDecision.MissingPair)
                {
                    self.Logger.LogWarning("[BMGHDIAG2] TRAVERSAL_INCONCLUSIVE id=" + sourceId + " native exitScript is null after TeleportPlayer(); no success bit recorded.");
                    return;
                }
                if (decision == TraversalDecision.InvalidPair)
                {
                    self.Logger.LogError("[BMGHDIAG2] TRAVERSAL_INCONCLUSIVE sourceId=" + sourceId + " targetId=" + targetId
                ...
                self.Logger.LogInfo("[BMGHDIAG2] TRAVERSED id=" + sourceId
                ...
                self.Logger.LogInfo("[BMGHDIAG2] COVERAGE " + coverage.Summary());
            ...
            catch (Exception ex)
            {
                ...
                self.Logger.LogError("[BMGHDIAG2] OBSERVER_INCONCLUSIVE; observation disabled, native gameplay left untouched: " + ex);
            }
        }
        ...
        private static void LogTopologySnapshot()
        {
            ...
            if (topology.IsOk)
            {
                self.Logger.LogInfo("[BMGHDIAG2] TOPOLOGY_OK ids=0,1,2,3 uniqueOppositePairs=4; " + topology.Summary
                    + "; active=" + orderedDetails);
            }
            else
            {
                self.Logger.LogError("[BMGHDIAG2] TOPOLOGY_INCONCLUSIVE " + topology.Summary + "; active=" + orderedDetails
                    + "; no repair attempted.");
            }
        }
