...
namespace S142ZCodeRebirthAerialDefenseSpawnTuning
{
    ...
    {
        internal const string PluginGuid = "tendas.lethalcompany.s142zcoderebirthaerialdefensespawntuning";
        internal const string PluginName = "S1.42Z CodeRebirth Aerial Defense Spawn Tuning";
        ...
        private void Awake()
        {
            ...
            {
                Logger.LogError("S1.42Z aerial-defense spawn tuning refused to arm because the validated CodeRebirth/DawnLib dependency set does not match the frozen baseline.");
                return;
            }
            ...
            Logger.LogInfo("S1.42Z aerial-defense spawn tuning armed; waiting for DawnLib moon-registry freeze to validate and scale both code_rebirth:air_control_unit and code_rebirth:gunslinger_greg curves by 0.5.");
        }
        ...
        private void ApplyAerialDefenseSpawnScale()
        {
            ...
            {
                Logger.LogError("S1.42Z aerial-defense spawn tuning refused to apply: DawnLib MapObjects registry was not frozen when the moon-freeze callback ran.");
                return;
            }
            ...
            foreach (TargetSpec target in Targets)
            {
                ...
                {
                    Logger.LogError("S1.42Z aerial-defense spawn tuning made no changes because the complete two-target contract did not validate.");
                    return;
                }
            ...
            applied = true;
            Logger.LogInfo(
                "S1.42Z aerial-defense spawn tuning applied transactionally: " +
        ...
        private bool TryValidateTarget(TargetSpec target, out ValidatedTarget validated)
        {
            ...
            {
                Logger.LogError($"S1.42Z aerial-defense spawn tuning contract failure: exact DawnLib map object code_rebirth:{target.MapObjectKey} ({target.Label}) with OutsideInfo was not found.");
                return false;
            }
            ...
            {
                Logger.LogError($"S1.42Z aerial-defense spawn tuning contract failure for {target.Label}: expected DawnLib ProviderTable private provider list was not found.");
                return false;
            }
            ...
            {
                Logger.LogError($"S1.42Z aerial-defense spawn tuning contract failure for {target.Label}: expected exactly one Dusk MapObjectSpawnMechanics provider, found {mechanics.Count}.");
                return false;
            }
            ...
            if (!actualKeys.SetEquals(target.ExpectedCurveKeys))
            {
                ...
                Logger.LogError($"S1.42Z aerial-defense spawn tuning contract failure for {target.Label}: curve-key contract drifted. Missing=[{missing}] Extra=[{extra}].");
                return false;
            }

            if (curves.Values.Any(curve => curve == null || curve.keys == null || curve.keys.Length == 0))
            {
                Logger.LogError($"S1.42Z aerial-defense spawn tuning contract failure for {target.Label}: one or more expected curves were null or empty.");
                return false;
            }
