...
LC Office scrap tuning remains unchanged. **LC Office Scrap Quantity/Distribution Investigation is the sole selected analysis scope** under `BuildSpecs/LC_OFFICE_SCRAP_INVESTIGATION_PLAN.md`. The count comparison in `Current/159_LC_OFFICE_SCRAP_EXISTING_EVIDENCE_FINDING.md` shows LC Office's 14-15 generated/final count is comparable to nearby normal Offense evidence; no low-count regression is established. Existing logs do not provide a complete item-to-room/floor map. The diagnostic designed in `Current/160_LC_OFFICE_SCRAP_PLACEMENT_DIAGNOSTIC_DESIGN.md` is now implemented and statically validated as S1.42AK-SCRAPDIAG1: exact S1.42AK plus the unchanged reviewed Office selector and a bounded read-only `RoundManager.SpawnScrapInLevel` placement logger. `Current/161_S1.42AK_SCRAPDIAG1_STATIC_VALIDATION_CLEARANCE.md` pins the exact review artifact for later publication. No runtime test or tuning is armed yet. Universal-moon availability and unrelated interior viability work remain separate scopes.
