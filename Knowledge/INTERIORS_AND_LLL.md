...
LC Office scrap tuning remains unchanged. **LC Office Scrap Quantity/Distribution Investigation is the sole selected analysis scope** under `BuildSpecs/LC_OFFICE_SCRAP_INVESTIGATION_PLAN.md`. The count comparison in `Current/159_LC_OFFICE_SCRAP_EXISTING_EVIDENCE_FINDING.md` shows LC Office's 14-15 generated/final count is comparable to nearby normal Offense evidence; no low-count regression is established. Existing logs do not provide a complete item-to-room/floor map. Exact published S1.42AK-SCRAPDIAG1 is now armed solely to close that placement gap: exact accepted S1.42AK plus the unchanged reviewed Office selector and a bounded read-only `RoundManager.SpawnScrapInLevel` placement logger. Publication identity is `BuildSpecs/S1.42AK-SCRAPDIAG1_BUILD_EVIDENCE/PUBLICATION_VERIFICATION.md`; runtime authority is `Current/162_S1.42AK_SCRAPDIAG1_RUNTIME_ACTIVATION.md`. The pending Offense run must produce stable placement markers before any tuning decision. Universal-moon availability and unrelated interior viability work remain separate scopes.
