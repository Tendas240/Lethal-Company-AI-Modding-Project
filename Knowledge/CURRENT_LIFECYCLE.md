...
The exact Black Mesa x Substation run at `RuntimeEvidence/S1.42AK-BMDSFIX1/20260924T170032Z/` remains the first preserved regular non-target control. Its severe stutter/freezing and delayed/overlapping landing symptom remains a separate runtime/performance observation and is not attributed to BMDSFIX1 because the pair-scoped patch did not apply.

A second regular exact-byte run is ingested at `RuntimeEvidence/S1.42AK-BMDSFIX1/20260925T092937Z/`, raw log SHA-256 `936858e50ec27fcbca9ca09b1b863ca1940d860ea3d44d2b5623431e221b9b6e`. BMDSFIX1 armed normally; Black Mesa returned a 31-flow viable pool normalized to effective rarity 100, including `Deep Sewers(100)`, but naturally selected `Expanded facility`. The dungeon completed and `Players finished generating the new floor` appeared. The user independently reported normal Black Mesa landing, acceptable `Entering the atmosphere` duration, normal movement, and no noticeable entrance/exit issue. This is an additional clean regular non-target control only and does not advance the target-selection requirement. Decision authority: `Current/187_S1.42AK_BMDSFIX1_REGULAR_EXPANDED_FACILITY_NON_TARGET_CONTROL.md`.

A later regular evidence set is ingested at `RuntimeEvidence/S1.42AK-BMDSFIX1/20260925T102825Z/`, raw log SHA-256 `779e55d5c248172da94bd1ddf6c5469c3bf78968f05912fb4ba3699cd335384c`. It contains two consecutive normal Black Mesa selections with the same 31-flow equal-rarity viable pool. Seed `71489917` selected `Slaughterhouse`; seed `14991801` also selected `Slaughterhouse`. Both generations reached `Players finished generating the new floor`. Neither run selected `DeepSewersFlow`, so neither advances the target qualification. No additional user gameplay claim is attached to this reconciliation. Decision authority: `Current/188_S1.42AK_BMDSFIX1_REGULAR_SLAUGHTERHOUSE_NON_TARGET_AND_SEED_ANALYSIS_PIVOT.md`.

The separately versioned `S1.42AK-BMDSFIX1-DIAG1PATH1` supporting run remains ingested at `RuntimeEvidence/S1.42AK-BMDSFIX1-DIAG1PATH1/20260925T085104Z/`, raw log SHA-256 `ec7631dc6baa3ef6cff502767d56356fe62943b362271f45add8807381a63ae4`. The short-name diagnostic armed without refusal, selected Black Mesa / `DeepSewersFlow` deterministically from the normalized viable pool (`pool=31->1`), BMDSFIX1 armed and applied `4.875->1`, final length multiplier was `1`, dungeon generation completed, `Players finished generating the new floor` appeared, normal post-generation activity continued, and the session later returned to orbit. The user independently reported no `Entering the atmosphere` problem and no noticeable gameplay issue. Decision authority: `Current/186_S1.42AK_BMDSFIX1_DIAG1PATH1_RUNTIME_SUPPORTING_PASS.md`.
...
`Current/189_S1.42AK_BMDSFIX1_SEED_SELECTION_ANALYSIS.md` closes the bounded seed-selection analysis requested by the Slaughterhouse pivot.

Installed-V81 evidence establishes that `GenerateNewLevelClientRpc` receives the map `randomSeed`, assigns it to `playersManager.randomMapSeed`, initializes the RNGs, and later `GenerateNewFloor()` calls `GetRandomWeightedIndex(..., LevelRandom)` before consuming a subsequent `LevelRandom.Next()` for the DunGen generator seed. LLL supplies the viable names/rarities in list order and delegates the final draw to this native path.

`Current/190_S1.42AK_BMDSFIX1_INSTALLED_V81_ROUNDMANAGER_SEED_CONTROL_RECONCILIATION.md` closes the exact-installed RoundManager RNG-body provenance. The integrated capture at `SourceEvidence/VanillaV81/RoundManagerSeedControl/20260925T175821Z-a1992708/` confirms `LevelRandom = new System.Random(playersManager.randomMapSeed)` in `InitializeRandomNumberGenerators()` and confirms that the positive-total-weight `GetRandomWeightedIndex(...)` path consumes one `NextDouble()` weighted draw.

`Current/191_S1.42AK_BMDSFIX1_INSTALLED_V81_STARTOFROUND_SEED_CONTROL_RECONCILIATION.md` closes the exact-installed upstream `StartOfRound` boundary using `SourceEvidence/VanillaV81/StartOfRoundSeedControl/20260926T154610Z-0b7be88d/`. `StartGame()` consumes either `overrideSeedNumber` when `overrideRandomSeed` is true, a deterministic weekly challenge seed, or `ChooseNewRandomMapSeed()`. The normal branch uses `UnityEngine.Random.Range(1, 100000000)`, and `OpenShipDoors()` forwards the resulting `randomMapSeed` into `RoundManager.LoadNewLevel(...)`. The assembly-wide decompiled scan contains only the field declarations and `StartGame()` consumption for both override fields; no `Assembly-CSharp.dll` writer for `overrideRandomSeed` or `overrideSeedNumber` is present in the captured evidence.

The selection-time Black Mesa pool contains 31 equal effective weights and places `Slaughterhouse` at 0-based slot `2`, `Expanded facility` at slot `7`, and `Deep Sewers` at slot `12`. The reconstructed model reproduces all three preserved regular observations:

- `71489917` -> first draw `0.07632330436088298` -> slot `2` -> Slaughterhouse;
- `14991801` -> first draw `0.07392922699168754` -> slot `2` -> Slaughterhouse;
- recovered `5996885` -> first draw `0.2565243683087287` -> slot `7` -> Expanded facility.

Primary candidate map seed **`1061420`** yields first reconstructed draw `0.4032298994265636`, which falls in the slot-12 Deep Sewers interval. Secondary candidates `1022959` and `1099881` also map to slot 12.

This establishes a justified seed prediction and ends the inefficient blind-reroll phase. The exact Vanilla seed path is now closed end-to-end, but it does **not** prove an ordinary deliberate Vanilla mechanism for entering arbitrary seed `1061420`: the normal branch is random, the challenge branch is week-derived, and the override fields have no writer in the exact decompiled `Assembly-CSharp.dll` scan. Before the target gameplay gate can be released, the next bounded review must determine whether the exact installed BMDSFIX1 profile already contains an ordinary non-diagnostic package/config/plugin mechanism outside Vanilla that can supply the seed through this native path without adding new code or altering native selection.
...
- Runtime test outstanding: **yes — regular exact-byte BMDSFIX1 Black Mesa x DeepSewersFlow gameplay qualification remains outstanding and unwaived, but is not yet released for execution**.
- Seed-selection reconstruction: **3/3 preserved regular observations reproduced; exact-installed RoundManager and StartOfRound seed path confirmed; primary Deep Sewers candidate seed `1061420`; Vanilla Assembly-CSharp contains no captured writer for the override fields; installed normal-stack seed-control availability review next**.
...
Perform a bounded repository-native **installed normal-stack seed-control availability review** for the exact S1.42AK-BMDSFIX1 profile. Determine whether any already-installed ordinary non-diagnostic package/config/plugin mechanism can deliberately supply map seed `1061420` through the native `StartOfRound`/`RoundManager` path without adding new code, selecting the dungeon directly, mutating the viable pool/weights, or consuming `LevelRandom` before native selection.

The exact Vanilla StartOfRound capture is already reconciled and contains no `Assembly-CSharp.dll` writer for `overrideRandomSeed` / `overrideSeedNumber`. If an existing ordinary mechanism is proven, reconcile it before releasing the regular Black Mesa x `DeepSewersFlow` target gameplay test, with the canonical Gale/import command when required and exact build-specific one-line PowerShell log uploader in the same test-instruction response. If no such mechanism exists, record the limitation and keep the target gate outstanding without returning to blind rerolls.

## Permanent Gale workflow

The canonical Gale helper remains `RuntimeTools/ReplaceActiveGaleProfileV24.ps1` at helper revision `2026-09-18-import-uia-v2.4.2-one-hop-diagnostic-parent-chain`. `RuntimeInbox/ACTIVE_BUILD.txt` points to `S1.42AK-BMDSFIX1`, so the launcher resolves the exact regular gameplay profile `Profiles/LC V1 S1.42AK-BMDSFIX1 Black Mesa Deep Sewers Size Fix.r2z` / SHA-256 `3f9c7fd5c21c532528db1ddae36764ada73236b7527c6ab2ae1b982c3976b7b0` through the normal active-build path. DIAG1PATH1 is historical supporting evidence only and must not be imported for the qualification run. Re-import is a recovery/identity-verification action, not a requirement between unchanged regular BMDSFIX1 retry runs.
