...
## BMDSFIX1 seed-selection analysis — reconstructed and closed-loop validated
...
`Current/190_S1.42AK_BMDSFIX1_INSTALLED_V81_ROUNDMANAGER_SEED_CONTROL_RECONCILIATION.md` supersedes the earlier RoundManager body-level provenance qualification. The integrated exact-installed capture at `SourceEvidence/VanillaV81/RoundManagerSeedControl/20260925T175821Z-a1992708/` confirms `LevelRandom = new System.Random(playersManager.randomMapSeed)` in `InitializeRandomNumberGenerators()` and confirms that the positive-total-weight `GetRandomWeightedIndex(...)` path consumes one `NextDouble()` weighted draw. The remaining exact-installed boundary is upstream in `StartOfRound`, not these RoundManager RNG bodies.
...
This establishes a justified seed prediction and ends the inefficient blind-reroll phase. The RoundManager RNG-body provenance gap is now closed by exact-installed evidence, but the runtime gate is still not ready: the next bounded proof must capture exact-installed `StartOfRound` seed origin/override reachability and determine whether an existing ordinary player-facing mechanism can supply the candidate seed without introducing a new seed writer.
...
- Seed-selection reconstruction: **3/3 preserved regular observations reproduced; exact-installed RoundManager RNG bodies confirmed; primary Deep Sewers candidate seed `1061420`; exact-installed StartOfRound seed-control capture next**.
...
Prepare and execute a bounded repository-native **Installed-V81 StartOfRound seed-control capture** against the same reviewed V81 identity. Capture `StartGame()`, `ChooseNewRandomMapSeed()`, `OpenShipDoors()`, the `overrideRandomSeed` / `overrideSeedNumber` fields, and direct assembly-level reads/writes/references sufficient to determine whether an existing ordinary player-facing mechanism can supply map seed `1061420` before `RoundManager.LoadNewLevel(...)`.

Do not run gameplay or build a seed setter, do not re-enable DIAG1PATH1 or add another dungeon selector, do not mutate the viable pool/weights, and do not consume `LevelRandom` before native selection. Reconcile that exact-installed evidence first. Only if it proves a clean ordinary mechanism may the regular Black Mesa x `DeepSewersFlow` target test be released, with the canonical Gale/import command when required and the exact build-specific one-line PowerShell log uploader in the same test-instruction response.
