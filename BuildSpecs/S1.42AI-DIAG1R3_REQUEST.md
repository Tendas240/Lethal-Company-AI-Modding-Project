...
The repository-native controller transition is bound to the temporary one-shot workflow `.github/workflows/s142ai-diag1r3-build-request.yml`. After its PR is validated and merged from the exact prepared main state, that workflow must fail closed on lifecycle/base/source drift, clean-compile the prepared diagnostic source, regenerate and bind the canonical disabled DIAG1 draft, refuse R3 output/snapshot reuse, transform only the exact R3 build identity into `BuildSpecs/current.json`, self-delete in the same controller-transition commit, keep runtime attribution unchanged, and dispatch the canonical `profile-build.yml` workflow.
